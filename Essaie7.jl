...
DA_ODE_true_instant
--------------------
Fonction biophysique de base du modèle de Fyon SANS contrôleur 
...
Caractéristiques :
  ...
  - Conductance de fuite calcique (gLCa) FIXE — découplée du 
    contrôleur homéostatique (du[31] = 0 et du[33] = 0)
...
function DA_homeo_ODE_Ntype_Ttype_FixedLeak_Ltype(du, u, p, t)
    ...
    gNa, gCaL, gKd, gKA, gKERG, gKSK, gH, gPace = clamp.(u[14:21], 0.0, g_max_main)
    gLNS, gLCa = clamp.(u[30:31], 0.0, g_max_leak)
    gNtype, gTtype = clamp.(u[34:35], 0.0, g_max_cal)
    ...
    # 8. CONTRÔLEUR HOMÉOSTATIQUE (du[14:33])
    error = (Ca_tgt - Ca)

    for k in 0:7 
        du[14+k] = (1/tau_g) * (u[22+k] - u[14+k]) 
    end
    
    du[30] = (1/tau_g) * (u[32] - u[30]) 
    du[31] = 0.0                             

    du[34] = (1/tau_g) * (u[36] - u[34])
    du[35] = (1/tau_g) * (u[37] - u[35])

    ts_main = [t_Na, tCaL_eff, tKd, tKA, tKERG, tKSK, tH, tPace]
    for k in 0:7
       
        if (u[14+k] >= g_max_main[k+1] && error > 0) || (u[22+k] <= 0.0 && error < 0)
            du[22+k] = 0.0
        else
    ...
    ts_cal = [tNCa, tTCA]
    for k in 0:1
        if (u[34+k] >= g_max_cal[k+1] && error > 0) || (u[36+k] <= 0.0 && error < 0)
            du[36+k] = 0.0
        else
    ...
    if (u[30] >= g_max_leak[1] && error > 0) || (u[32] <= 0.0 && error < 0)
        du[32] = 0.0
    else
        du[32] = (1/tLNS) * error
    end
...
Caractéristiques :
  - Identique à DA_homeo_ODE_Ntype_Ttype_FixedLeak_Ltype 
  ...
  - gLCa fixe (du[31] = 0), gLNS régulé normalement
"""

function DA_homeo_ODE_Hblock(du, u, p, t)
    # 1. PARAMÈTRES
    Iapp   = p[1](t)
    Ca_tgt = p[2]
    tau_g  = p[3]
    ...
    # ---  DÉPENDANCE AU CALCIUM (SK & Pompe) ---
    SK_inf = 0.0
    ICap = 0.0
    if Ca > 0
        SK_inf = 1 / (1 + (0.00019 / Ca)^4)
    ...
    ts_main = [t_Na, tCaL, tKd, tKA, tKERG, tKSK, tH_eff, tPace]
    for k in 0:7
        if (u[14+k] >= g_max_main[k+1] && error > 0) || (u[22+k] <= 0.0 && error < 0)
            du[22+k] = 0.0
        else
...
function DA_homeo_ODE_Ablock(du, u, p, t)
    # 1. PARAMÈTRES
    Iapp   = p[1](t)
    Ca_tgt = p[2]
    tau_g  = p[3]
    ...
    ts_main = [t_Na, tCaL, tKd, tKA_eff, tKERG, tKSK, tH, tPace]
    for k in 0:7
        if (u[14+k] >= g_max_main[k+1] && error > 0) || (u[22+k] <= 0.0 && error < 0)
            du[22+k] = 0.0
        else
...
DA_homeo_gPace
---------------
Fonction pour simuler le BLOCAGE DE LA CONDUCTANCE gPace (IPace/IXG)
avec contrôleur homéostatique intégré.

Caractéristiques :
  - Identique à DA_homeo_ODE_Ntype_Ttype_FixedLeak_Ltype 
    dans sa structure générale
  - Blocage instantané de gPace à t = 30000 ms 
  - Tous les autres canaux, y compris gCaL, restent actifs 
    et régulés normalement
  - gLCa fixe (du[31] = 0), gLNS régulé normalement
"""
function DA_homeo_gPace(du, u, p, t)
    # 1. PARAMÈTRES
    Iapp   = p[1](t)
    Ca_tgt = p[2]
    tau_g  = p[3]
    ...
    # 3. CONDUCTANCES ET mRNA
    gs_main = clamp.(u[14:21], 0.0, g_max_main)
    gs_leak = clamp.(u[30:31], 0.0, g_max_leak)
    gs_cal  = clamp.(u[34:35], 0.0, g_max_cal)
